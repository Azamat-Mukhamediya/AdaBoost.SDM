...
class LapRLS(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        if self.solver == 'closed-form':
            ...
            def RLS(alpha):
                return np.squeeze(
                    np.array(
                        (1 / l) * (self.Y - J.dot(K).dot(alpha)).T.dot(
                            (self.Y - J.dot(K).dot(alpha))) + self.lambda_k * alpha.dot(K).dot(alpha)
                        + (self.lambda_u / n ** 2) * alpha.dot(K).dot(L).dot(K).dot(alpha)))

            def RLS_grad(alpha):
                return np.squeeze(np.array(grad_part1 + grad_part2.dot(alpha)))

            self.alpha, _, _ = sco.fmin_l_bfgs_b(
                RLS, x0, RLS_grad, args=(),
